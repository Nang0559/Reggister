    ...
    public class OTController : BaseApiController
    {
            ...
            [FromQuery] string? deptCode, CancellationToken ct)
        {
            ...
            if (string.IsNullOrWhiteSpace(dept))
                dept = await _queryService.GetEmployeeDeptCodeAsync(UserInfo.EmployeeCode ?? "", ct);
            if (string.IsNullOrWhiteSpace(dept))
            ...
            CancellationToken ct = default)
        {
            ...
            if (!string.IsNullOrWhiteSpace(status) &&
                Enum.TryParse<ApprovalStatus>(status, true, out var statusValue))
            ...
            var result = await _queryService.GetPagedAsync(
                ...
                new DateTime(year ?? DateTime.Now.Year, 12, 31),
                page,
                pageSize,
                ct);
        ...
        {
            if (UserInfo == null) return Unauthorized(ApiResponse<object>.Fail("Phiên đăng nhập hết hạn."));
            var result = await _queryService.GetBalanceAsync(
        ...
        {
            if (UserInfo == null) return Unauthorized(ApiResponse<object>.Fail("Phiên đăng nhập hết hạn."));
            var result = await _queryService.GetSimpleBalanceAsync(
        ...
        public async Task<IActionResult> GetDashboard(CancellationToken ct)
        {
            ...
            var balance = await _queryService.GetSimpleBalanceAsync(
                UserInfo.EmployeeCode ?? "", DateTime.Now.Year, ct);
            var recent = await _queryService.GetRecentSummaryAsync(
            ...
            CancellationToken ct = default)
        {
            ...
            var data = await _queryService.GetPagedAsync(
                deptCode, parsedStatus, fromDate, toDate, page, pageSize, ct);
        ...
        public async Task<IActionResult> GetPendingSummary(CancellationToken ct)
        {
            ...
            var summary = items
                .GroupBy(x => x.CurrentApprovalLevel)
                .OrderBy(x => x.Key)
            ...
            [FromBody] OTRequestUpsertDto dto, CancellationToken ct)
        {
            ...
            await LogActionAsync($"Tạo đơn OT ngày {dto.OTDate:dd/MM/yyyy} - {dto.Employees.Count} nhân viên");
        ...
        public async Task<IActionResult> UpdateEmployeeOTInfo(
            int id, [FromBody] List<OTEmployeeDto> employees, CancellationToken ct)
        {
            if (UserInfo == null) return Unauthorized(ApiResponse<object>.Fail("Phiên đăng nhập hết hạn."));
