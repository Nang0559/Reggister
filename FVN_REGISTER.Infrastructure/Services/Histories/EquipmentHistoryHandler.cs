...
public sealed class EquipmentHistoryHandler : BaseHistoryHandler<F03EquipmentRequest>
{
    protected override RequestModule ModuleKind => RequestModule.Equipment;

    public EquipmentHistoryHandler(FVNWEBAPPContext db) : base(db) { }

    protected override System.Linq.Expressions.Expression<Func<F03EquipmentRequest, bool>> BuildIdPredicate(int id)
        => x => x.Id == id;
    ...
    {
        var q = from request in Db.EquipmentRequests.AsNoTracking()
        ...
        if (filter.FromDate.HasValue)
            q = q.Where(x => x.request.CreatedAt >= filter.FromDate.Value);
        if (filter.ToDate.HasValue)
            q = q.Where(x => x.request.CreatedAt <= filter.ToDate.Value);
        if (!string.IsNullOrWhiteSpace(filter.SearchText))
        {
            var s = filter.SearchText.ToLower();
            q = q.Where(x => x.request.EquipmentName.ToLower().Contains(s) ||
                             (x.request.AssetCode != null && x.request.AssetCode.ToLower().Contains(s)));
        }
        ...
        var items = data.Select(x => new HistoryItemDto
        {
            ...
            Reason = x.request.Note ?? x.request.RepairContent ?? x.request.EquipmentName,
    ...
    {
        var row = await (from request in Db.EquipmentRequests.AsNoTracking()
                         join employee in Db.Employees.AsNoTracking()
                             on request.EmployeeCode equals employee.EmployeeCode into employees
                         from employee in employees.DefaultIfEmpty()
                         where request.Id == id &&
        ...
        if (row == null)
            return ServiceResult<HistoryItemDetailDto>.Fail("Không tìm thấy đơn thiết bị.");

        return ServiceResult<HistoryItemDetailDto>.Ok(new HistoryItemDetailDto
        {
            Id = id,
            Kind = RequestModule.Equipment.ToString(),
            ...
            Equipment = new EquipmentDetailPayload
            {
                ...
                PurchaseDate = row.request.PurchaseDate,
                ExpectedDepreciationDate = row.request.ExpectedDepreciationDate,
                RepairDate = row.request.RepairDate,
                RepairContent = row.request.RepairContent,
                RepairVendor = row.request.RepairVendor,
                RepairCost = row.request.RepairCost,
                RepairResult = row.request.RepairResult,
                Specification = row.request.Specification,
                SerialNumber = row.request.SerialNumber,
