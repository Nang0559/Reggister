    ...
    using global::FVN_REGISTER.Application.Services.Common;
        ...
        ILeaveService
        {
                ...
                LeaveRequestUpsertDto model, UserIdentityDto user, CancellationToken ct = default)
            {
                try
                {
                    ...
                    Logger.LogInfoIf(Debug, "[{Component}] Created LeaveId={Id} By={User}",
            ...
            // ================= UPDATE (khai báo ở ILeaveService, chưa có trong bản gốc bạn gửi) =================
            public  async Task<ServiceResult<int>> UpdateAsync(
            LeaveRequestUpsertDto model, UserIdentityDto user, CancellationToken ct = default)
            {
                try
                {
                    Logger.LogDebugIf(Debug, "[{Component}] UpdateAsync start: Id={Id} User={User}",
                        ComponentName, model.Id, user.EmployeeCode);
                    ...
                    // Chặn cứng: snapshot approval PERSIST 1 LẦN, KHÔNG BAO GIỜ SỬA SAU (nguyên tắc D2 Bước 2).
                    // Nếu đã có bất kỳ quyết định duyệt nào, không cho sửa nội dung đơn — tránh lệch giữa
                    // dữ liệu mới và hierarchy approver đã build từ dữ liệu cũ.
                    if (await HasAnyApprovalDecisionAsync(entity.Id, ct))
                        return ServiceResult<int>.Fail(
                            "Đơn đã có bước duyệt được xử lý, không thể sửa nội dung. Vui lòng hủy đơn và tạo lại nếu cần thay đổi.");
                    ...
                    try
                    {
                        // Thay toàn bộ chi tiết cũ — đơn giản và an toàn hơn diff từng dòng,
                        // vì LeaveRequestUpsertDto.Details không mang Id riêng để đối chiếu.
                        var detailRepo = Uow.Repository<F03LeaveDayDetail>();
                        ...
                        Logger.LogInfoIf(Debug, "[{Component}] Updated LeaveId={Id} By={User}",
                            ComponentName, entity.Id, user.EmployeeCode);
            ...
            /// <summary>
            /// Kiểm tra đơn đã có bất kỳ quyết định duyệt nào chưa (F03ApprovalHistory append-only).
            /// Dùng để chặn Update khi snapshot approver đã "sống" — tránh lệch dữ liệu vs người duyệt.
            /// </summary>
                ...
                try
                {
                    ...
                    if (remaining == 0)
                    {
                        ...
                        Logger.LogInfoIf(Debug,
                            "[{Component}] Last detail removed -> parent LeaveId={Id} auto-cancelled",
                            ComponentName, parent.Id);
                    ...
                    Logger.LogInfoIf(Debug, "[{Component}] CancelDetail DetailId={Id} By={User}",
                        ComponentName, detailId, user.EmployeeCode);
