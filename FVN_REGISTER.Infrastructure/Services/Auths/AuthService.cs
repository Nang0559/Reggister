...
namespace FVN_REGISTER.Infrastructure.Services.Auths
{
    public class AuthService : BaseService<AuthService>, IAuthService
    {
        ...
        private async Task<string> GenerateJwtTokenAsync(F03User user,bool rememberMe,CancellationToken ct){var key=_jwtOptions.SecretKey??string.Empty;if(key.Length<32)throw new InvalidOperationException("JWT SecretKey phải có ít nhất 32 ký tự.");var emp=await _uow.Repository<F03Employee>().Query().AsNoTracking().FirstOrDefaultAsync(x=>x.EmployeeCode==user.EmployeeCode,ct);var claims=new List<Claim>{new(JwtRegisteredClaimNames.Sub,user.Id.ToString()),new(JwtRegisteredClaimNames.UniqueName,user.EmployeeCode??string.Empty),new(ClaimTypes.Name,user.FullName??user.EmployeeCode??string.Empty),new("EmployeeCode",user.EmployeeCode??string.Empty),new("DeptCode",user.DeptCode??string.Empty),new("Permission",user.PermissionCode??string.Empty),new("LevelApprove",user.LevelApprove?.ToString()??string.Empty),new("PositionCode",emp?.PositionCode??string.Empty)};var creds=new SigningCredentials(new SymmetricSecurityKey(Encoding.UTF8.GetBytes(key)),SecurityAlgorithms.HmacSha256);var token=new JwtSecurityToken(issuer:_jwtOptions.Issuer,audience:_jwtOptions.Audience,claims:claims,expires:DateTime.UtcNow.AddMinutes(_jwtOptions.AccessTokenMinutes),signingCredentials:creds);return new JwtSecurityTokenHandler().WriteToken(token);}
