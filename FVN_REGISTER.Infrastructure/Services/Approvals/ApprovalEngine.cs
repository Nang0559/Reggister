...
{
    public class ApprovalEngine<TSubject> : IApprovalEngine<TSubject> where TSubject : IApprovalSubject
    {
        ...
        // ================= 1. TẠO SNAPSHOT KHI NỘP ĐƠN =================
        public async Task InitializeStepsAsync(int requestId, ApprovalSnapshotDto snapshot, CancellationToken ct)
        {
            var entity = new F03ApprovalSnapshot
            {
                ...
                Steps = snapshot.Steps
                    ...
                    .Select(s => new F03ApprovalStepSnapshot
                    {
                        ...
                        ApproverEmail = s.ApproverEmail,   // SỬA: thêm dòng này
                        RoleName = s.RoleName,
        ...
        // ================= 2. DUYỆT / TỪ CHỐI =================
        public async Task<ApprovalActionResult> ProcessDecisionAsync(ApprovalActionDto action, CancellationToken ct)
        {
            ...
            foreach (var requestId in action.RequestIds)
            {
                ...
                bool isOverride = !string.Equals(action.ApproverCode, step.ApproverCode, StringComparison.OrdinalIgnoreCase);
            ...
            foreach (var requestId in processedIds)
            {
                ...
                {
                    // Dùng ĐÚNG kết quả ApplyOverallStatusAsync vừa persist — không tự suy luận lại
                    bool isFullyApproved = subject.OverallStatus == ApprovalStatus.Approved;

                    await _provider.NotifyStepCompletedAsync(subject, completedStep, isFullyApproved, ct);
                }
            }
        ...
        // ================= 3. DANH SÁCH CHỜ DUYỆT (theo email → resolve sang code) =================
        public async Task<List<PendingApprovalItemDto>> GetPendingForApproverAsync(string approverEmail, CancellationToken ct)
        {
            ...
            var snapshots = await _uow.Repository<F03ApprovalSnapshot>()
                .Query()
                .Include(s => s.Steps)
                .Where(s => s.RequestType == Module && s.Steps.Any(step => step.ApproverCode == approverCode))
                .ToListAsync(ct);
            ...
            foreach (var snapshot in snapshots)
            {
                ...
                var item = await _provider.ToPendingItemAsync(subject, calculated, canApprove: true, ct);
                result.Add(item);
            }
        ...
        // ================= 4. TÍNH TOÁN STEPS (SNAPSHOT + HISTORY) =================
        public async Task<List<ApprovalStepCalculatedDto>> GetStepsAsync(int requestId, CancellationToken ct)
        {
            var snapshot = await _uow.Repository<F03ApprovalSnapshot>()
                .Query()
                .Include(s => s.Steps)
        ...
        // ================= 5. BƯỚC TIẾP THEO =================
        public async Task<ApprovalStepDto?> GetNextStepAsync(int requestId, CancellationToken ct)
        {
            var snapshot = await _uow.Repository<F03ApprovalSnapshot>()
                .Query()
                .Include(s => s.Steps)
            ...
            // SỬA: bỏ query F03Employee thừa — đọc thẳng ApproverEmail đã lưu bất biến
            // trong snapshot, đúng nguyên tắc "snapshot ghi 1 lần, không đổi theo dữ liệu HRM sau này"
