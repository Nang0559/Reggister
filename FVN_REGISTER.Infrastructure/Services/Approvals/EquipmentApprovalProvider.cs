...
public sealed class EquipmentApprovalProvider : BaseApprovalProvider<EquipmentRequestSubject, EquipmentApprovalProvider>, IApprovalProvider<EquipmentRequestSubject>
{
    ...
    {
        (1, "Level 1", "EquipmentApprover"), (2, "Level 2", "EquipmentManager"), (3, "Level 3", "GM")
    };

    public EquipmentApprovalProvider(IUnitOfWork uow, IEmailService email, IApprovalNotificationService notification,
        IEmployeeUserResolver userResolver, ILogger<EquipmentApprovalProvider> logger, IOptionsMonitor<AuthDebugOptions> options)
            ...
            if (def.Level == 1 && !string.IsNullOrWhiteSpace(ctx.SelectedApproverCode))
                approver = await _uow.Repository<F03Approver>().Query().AsNoTracking()
                    .FirstOrDefaultAsync(x => x.RequestType == RequestType && x.IsActive == true
                        ...
                        && (x.ApproveForDeptCode == ctx.DeptCode || x.ApproveForDeptCode == ApproveForDept.All), ct);
            else
    ...
    public override async Task<EquipmentRequestSubject?> GetSubjectAsync(int requestId, CancellationToken ct)
    {
        ...
        var e = await _uow.Repository<F03Employee>().Query().AsNoTracking()
            .Where(v => v.EmployeeCode == x.EmployeeCode).Select(v => new { v.EmployeeName, v.PositionCode }).FirstOrDefaultAsync(ct);
    ...
    public override async Task<List<EquipmentRequestSubject>> GetSubjectsAsync(List<int> requestIds, CancellationToken ct)
    {
        ...
        var employees = await _uow.Repository<F03Employee>().Query().AsNoTracking().Where(x => codes.Contains(x.EmployeeCode))
            .Select(x => new { x.EmployeeCode, x.EmployeeName, x.PositionCode }).ToDictionaryAsync(x => x.EmployeeCode, x => x, ct);
    ...
    public override async Task ApplyOverallStatusAsync(int requestId, IReadOnlyList<ApprovalStepCalculatedDto> allSteps, CancellationToken ct)
    {
        ...
        entity.RequestStatus = required.Any(x => x.Status == DecisionType.Rejected) ? ApprovalStatus.Rejected
            : required.Count > 0 && required.All(x => x.Status == DecisionType.Approved) ? ApprovalStatus.Approved
        ...
        if (entity.RequestStatus == ApprovalStatus.Approved)
        {
            if (entity.RequestKind == EquipmentRequestKind.Registration && entity.AssetId == null)
            {
                ...
                {
                    EquipmentCode = $"EQP-{Guid.NewGuid():N}"[..30], EquipmentName = entity.EquipmentName,
                    Specification = entity.Specification, SerialNumber = entity.SerialNumber, AssetCode = entity.AssetCode,
                    PurchasePrice = entity.PurchasePrice, PurchaseDate = entity.PurchaseDate!.Value,
                    ExpectedDepreciationDate = entity.ExpectedDepreciationDate!.Value, DeptCode = entity.DeptCode!,
                    Location = entity.Location, QrToken = entity.QrToken, IsQrActive = true, IsActive = true,
                    Note = entity.Note, CreatedBy = entity.OperatorUserId
                };
            ...
            else if (entity.RequestKind == EquipmentRequestKind.Repair && entity.AssetId.HasValue && entity.RepairDate.HasValue)
            {
                var exists = await _uow.Repository<F03EquipmentRepairHistory>().Query()
                    .AnyAsync(h => h.RequestId == entity.Id, ct);
                if (!exists)
                    await _uow.Repository<F03EquipmentRepairHistory>().AddAsync(new F03EquipmentRepairHistory
                    {
                        AssetId = entity.AssetId.Value, RequestId = entity.Id, RepairDate = entity.RepairDate.Value,
                        OperatorUserId = entity.OperatorUserId, RepairCost = entity.RepairCost,
                        RepairContent = entity.RepairContent ?? string.Empty, RepairVendor = entity.RepairVendor,
                        RepairResult = entity.RepairResult, Note = entity.Note, IsApproved = true, CreatedBy = entity.OperatorUserId
                    }, ct);
            }
        }
        await _uow.SaveChangesAsync(ct);
    ...
    public override async Task NotifyStepCompletedAsync(EquipmentRequestSubject subject, ApprovalStepCalculatedDto completedStep, bool isFullyApproved, CancellationToken ct)
    {
        ...
        var email = await _uow.Repository<F03Employee>().Query().AsNoTracking().Where(x => x.EmployeeCode == subject.EmployeeCode)
        ...
        if (!string.IsNullOrWhiteSpace(email))
            await _email.QueueEmail(email, isFullyApproved ? "EQUIPMENT_APPROVED" : "EQUIPMENT_REJECTED",
                new { subject.RequestId, subject.RequestKind, subject.EquipmentName, subject.AssetCode, subject.RepairDate,
                    Status = isFullyApproved ? "Approved" : "Rejected" }, ct);
    ...
    {
        var deptName = await _uow.Repository<F03Department>().Query().AsNoTracking().Where(x => x.DeptCode == subject.DeptCode)
            .Select(x => x.DeptName).FirstOrDefaultAsync(ct);
        return new PendingApprovalItemDto
        {
            ...
            ToDate = subject.RepairDate ?? DateTime.Now, TotalUnits = 1, ApprovalSteps = steps, CanApprove = canApprove
        };
    }
}
