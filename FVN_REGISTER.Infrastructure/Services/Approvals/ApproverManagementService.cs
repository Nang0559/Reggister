...
using FVN_REGISTER.Core.Constants.FVN_REGISTER.Core.Constants;
...
{
    public class ApproverManagementService
      : BaseService<ApproverManagementService>, IApproverManagementService
    {
        ...
        public async Task<ServiceResult<List<ApproverTreeNodeDto>>> GetApproverTreeAsync(CancellationToken ct)
        {
            try
            {
                var all = await _uow.Repository<F03Approver>()
                    .Query()
                    .AsNoTracking()
                    .Where(x => x.IsActive == true)
                    .OrderBy(x => x.ApproveForDeptCode)
                    .ThenBy(x => x.Level)
                ...
                var deptCodes = all
                    .Select(x => x.ApproveForDeptCode)
                    .Where(x => x != ApproveForDept.All)
                    .Distinct()
                    .ToList();

                var deptNames = await _uow.Repository<F03Department>()
                    .Query()
                    .AsNoTracking()
                ...
                var tree = all
                    .GroupBy(x => x.ApproveForDeptCode)
            ...
            string? deptCode, int? level, RequestModule? requestType, CancellationToken ct)
        {
            try
            {
                var q = _uow.Repository<F03Approver>()
                    .Query()
                    .AsNoTracking()
                    .Where(x => x.IsActive == true);

                if (!string.IsNullOrEmpty(deptCode))
                    q = q.Where(x => x.ApproveForDeptCode == deptCode
        ...
        public async Task<ServiceResult<List<DepartmentDto>>> GetDepartmentsAsync(CancellationToken ct)
        {
            try
            {
                var depts = await _uow.Repository<F03Department>()
                    .Query()
                    .AsNoTracking()
                    .Where(x => x.IsActive)
            ...
            string? deptCode, CancellationToken ct)
        {
            try
            {
                var q = _uow.Repository<F03Employee>().Query().AsNoTracking().AsQueryable();
        ...
        public async Task<ServiceResult> CreateAsync(ApproverDto model, int currentUserId, CancellationToken ct)
        {
            try
            {
                ...
                bool exists = await repo.Query().AnyAsync(x =>
                    x.ApproverCode == model.ApproverCode
                 && x.Level == model.Level
                 && x.RequestType == model.RequestType
                 && x.ApproveForDeptCode == model.ApproveForDeptCode
                 && x.IsActive == true, ct);
                ...
                int resolvedLevel = model.Level > 0
                    ? model.Level
                    : CvCodeRules.ResolveLevel(null, empInfo?.PositionCode);
        ...
        public async Task<ServiceResult> UpdateAsync(ApproverDto model, int currentUserId, CancellationToken ct)
        {
            try
            {
                ...
                // Có tracking (không AsNoTracking) vì cần SaveChanges sau khi sửa entity
                ...
                if (empInfo != null)
                {
                    entity.CvCode = empInfo.PositionCode ?? entity.CvCode;
        ...
        public async Task<int> GetRequesterLevelAsync(string employeeCode, RequestModule requestType, CancellationToken ct)
        {
            ...
            var levelFromTable = await _uow.Repository<F03Approver>()
                .Query()
        ...
        {
            var code = requestType.ToCode();

            return await _uow.Repository<F03Approver>()
                .Query()
                .AsNoTracking()
                .Where(x => x.ApproverCode == employeeCode && x.RequestType == requestType && x.IsActive == true)
        ...
        {
            if (string.IsNullOrWhiteSpace(model.ApproverCode))
                return ServiceResult.Fail("Chưa chọn nhân viên.");

            if (model.RequestType != RequestModule.Leave
                 && model.RequestType != RequestModule.Overtime)
        ...
        private async Task<string> GetDeptNameAsync(string deptCode, CancellationToken ct)
        {
            ...
            return await _uow.Repository<F03Department>()
                .Query()
        ...
        private async Task<EmployeeWithPositionInfo?> GetEmployeeWithPositionAsync(string employeeCode, CancellationToken ct)
        {
            var empQuery = _uow.Repository<F03Employee>().Query().AsNoTracking();
            var deptQuery = _uow.Repository<F03Department>().Query().AsNoTracking();
            var posQuery = _uow.Repository<F03Position>().Query().AsNoTracking();

            var result = await empQuery
                .Where(e => e.EmployeeCode == employeeCode)
                .Join(deptQuery,
                    e => e.DeptCode, d => d.DeptCode,
                    (e, d) => new { e, d.DeptName })
                .GroupJoin(posQuery,
                    x => x.e.PositionCode, p => p.PositionCode,
                    (x, positions) => new { x.e, x.DeptName, positions })
                .SelectMany(x => x.positions.DefaultIfEmpty(), (x, p) => new { x.e, x.DeptName, Position = p })
                .FirstOrDefaultAsync(ct);
