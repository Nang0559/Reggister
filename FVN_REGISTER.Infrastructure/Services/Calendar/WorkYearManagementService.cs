...
public sealed class WorkYearManagementService : IWorkYearManagementService
{
    ...
    public Task<List<WorkYearDto>> GetAllAsync(CancellationToken ct = default) =>
        _uow.Repository<F03WorkYear>().Query().AsNoTracking()
    ...
    public async Task<ServiceResult<WorkYearDto>> CreateAsync(WorkYearDto model, int userId, CancellationToken ct = default)
    {
        ...
        if (await repo.Query().AnyAsync(x => x.WorkYear == model.Year, ct))
            return ServiceResult<WorkYearDto>.Fail($"Năm {model.Year} đã tồn tại.");

        if (model.IsActive)
            await repo.Query().Where(x => x.IsActive == true).ExecuteUpdateAsync(s => s.SetProperty(x => x.IsActive, false), ct);

        var entity = new F03WorkYear
        {
            WorkYear = model.Year,
        ...
        return ServiceResult<WorkYearDto>.Ok(ToDto(entity));
    }
    ...
    public async Task<ServiceResult<WorkYearDto>> UpdateAsync(WorkYearDto model, int userId, CancellationToken ct = default)
    {
        ...
        if (entity == null) return ServiceResult<WorkYearDto>.Fail("Không tìm thấy năm làm việc.");
        if (model.EndDate.Date < model.StartDate.Date) return ServiceResult<WorkYearDto>.Fail("Ngày kết thúc phải >= ngày bắt đầu.");

        if (await repo.Query().AnyAsync(x => x.Id != model.Id && x.WorkYear == model.Year, ct))
            return ServiceResult<WorkYearDto>.Fail($"Năm {model.Year} đã tồn tại.");

        if (model.IsActive)
            await repo.Query().Where(x => x.Id != model.Id && x.IsActive == true).ExecuteUpdateAsync(s => s.SetProperty(x => x.IsActive, false), ct);

        entity.WorkYear = model.Year;
    ...
    public async Task<ServiceResult> SetActiveAsync(int id, bool active, int userId, CancellationToken ct = default)
    {
        ...
        if (entity == null) return ServiceResult.Fail("Không tìm thấy năm làm việc.");

        if (active)
            await repo.Query().Where(x => x.Id != id && x.IsActive == true).ExecuteUpdateAsync(s => s.SetProperty(x => x.IsActive, false), ct);
    ...
    private static WorkYearDto ToDto(F03WorkYear x) => new()
    {
        Id = x.Id, Year = x.WorkYear, YearName = $"Năm {x.WorkYear}",
        StartDate = x.StartDate, EndDate = x.EndDate, IsActive = x.IsActive == true, Remark = x.Remark
    };
}