...
{
    private readonly IUnitOfWork _uow; private readonly ICurrentUserService _currentUser; private readonly IApprovalWorkflowOrchestrator<EquipmentRequestSubject> _workflow; private readonly IAuthorizationService _authorization;
    public EquipmentService(IUnitOfWork uow, ICurrentUserService currentUser, IApprovalWorkflowOrchestrator<EquipmentRequestSubject> workflow, IAuthorizationService authorization) { _uow = uow; _currentUser = currentUser; _workflow = workflow; _authorization = authorization; }
    ...
    { var user = RequireModuleUser(); if (string.IsNullOrWhiteSpace(qrToken)) throw new ArgumentException("QR token không hợp lệ."); var asset = await _uow.Repository<F03EquipmentAsset>().Query().AsNoTracking().FirstOrDefaultAsync(x => x.QrToken == qrToken && x.IsActive == true, ct); if (asset == null || !asset.IsQrActive) throw new KeyNotFoundException("QR chưa có hiệu lực hoặc thiết bị không tồn tại."); await EnsureScopeAsync(user, SecurityFunctionCodes.EquipmentView, user.EmployeeCode, asset.DeptCode, ct); return await MapAssetAsync(asset, ct); }
