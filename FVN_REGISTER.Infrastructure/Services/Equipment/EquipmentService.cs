...
public sealed class EquipmentService : IEquipmentService
{
    ...
    { var user = RequireModuleUser(); if (string.IsNullOrWhiteSpace(qrToken)) throw new ArgumentException("QR token không hợp lệ."); var asset = await _uow.Repository<F03EquipmentAsset>().Query().AsNoTracking().FirstOrDefaultAsync(x => x.QrToken == qrToken && x.IsActive == true, ct); if (asset == null || !asset.IsQrActive) throw new KeyNotFoundException("QR chưa có hiệu lực hoặc thiết bị không tồn tại."); return await MapAssetAsync(asset, ct); }
