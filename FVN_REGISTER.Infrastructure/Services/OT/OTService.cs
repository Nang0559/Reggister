           ...
           IOTService
    {
            ...
            OTRequestUpsertDto model, UserIdentityDto user, CancellationToken ct = default)
        {
            try
            {
                ...
                if (model.Employees == null || model.Employees.Count == 0)
                    return ServiceResult<int>.Fail("Phải chọn ít nhất 1 nhân viên.");
                ...
                {
                    EmployeeCode = user.EmployeeCode ?? "", // người tạo đơn
                ...
                await Uow.SaveChangesAsync(ct); // cần Id trước khi thêm employees
                ...
                Logger.LogInfoIf(Debug, "[{Component}] Created OTId={Id} By={User}",
                    ComponentName, entity.Id, user.EmployeeCode);

                return ServiceResult<int>.Ok(entity.Id, "Đã tạo đơn tăng ca thành công.");
            }
            catch (Exception ex)
            {
                return InternalError<int>(ex, "Lỗi hệ thống khi tạo đơn tăng ca.");
            }
        }

        // ================= JOIN =================
        public async Task<ServiceResult> JoinAsync(
            int otRequestId, UserIdentityDto user, CancellationToken ct = default)
        {
            try
            {
                ...
                var empRepo = Uow.Repository<F03OTEmployee>();
                var exists = await empRepo.Query()
                    .AnyAsync(x => x.OTRequestId == otRequestId && x.EmployeeCode == user.EmployeeCode && x.IsActive==true, ct);

                if (exists)
                    return ServiceResult.Fail("Bạn đã có trong đơn này.");
                ...
                Logger.LogInfoIf(Debug, "[{Component}] {User} joined OT Id={Id}",
                    ComponentName, user.EmployeeCode, otRequestId);

                return ServiceResult.Ok("Đã tham gia đơn tăng ca.");
            }
            catch (Exception ex)
            {
                return InternalError(ex, "Lỗi hệ thống khi tham gia đơn tăng ca.");
            }
        }

        // ================= REMOVE EMPLOYEE =================
        public async Task<ServiceResult> RemoveEmployeeAsync(
      int otRequestId, string employeeCode, UserIdentityDto user, CancellationToken ct = default)
        {
            try
            {
                ...
                var otRepo = Uow.Repository<F03OTRequest>();
                var entity = await otRepo.GetByIdAsync(otRequestId, ct);
                if (entity == null) return ServiceResult.Fail("Không tìm thấy đơn OT.");

                var empRepo = Uow.Repository<F03OTEmployee>();
                var row = await empRepo.Query()
                    .FirstOrDefaultAsync(x => x.OTRequestId == otRequestId && x.EmployeeCode == employeeCode && x.IsActive == true, ct);
                if (row == null) return ServiceResult.Fail("Không tìm thấy nhân viên trong đơn.");

                bool isSelf = employeeCode == user.EmployeeCode;
                bool isCreator = entity.EmployeeCode == user.EmployeeCode;   // SỬA: cho phép người tạo đơn xóa thành viên
                if (!isSelf && !isCreator && !IsAdmin(user))
                    return ServiceResult.Fail("Không có quyền xóa nhân viên này khỏi đơn.");
            ...
            {
                return InternalError(ex, "Lỗi hệ thống khi xóa nhân viên khỏi đơn.");
            }
        }

        // ================= UPDATE EMPLOYEE OT INFO =================
        public async Task<ServiceResult> UpdateEmployeeOTInfoAsync(
            int otRequestId, List<OTEmployeeDto> updates, UserIdentityDto user, CancellationToken ct = default)
        {
            if (updates == null || updates.Count == 0)
                return ServiceResult.Fail("Không có dữ liệu cập nhật.");
            ...
            try
            {
                ...
                if (IsFinalized(entity) && !IsAdmin(user))
                    return ServiceResult.Fail("Đơn đã xử lý xong, không thể cập nhật.");

                var empRepo = Uow.Repository<F03OTEmployee>();
                var codes = updates.Select(u => u.EmployeeCode).ToList();

                var rows = await empRepo.Query()
                    .Where(x => x.OTRequestId == otRequestId && codes.Contains(x.EmployeeCode) && x.IsActive==true)
                    .ToListAsync(ct);
            ...
            {
                return InternalError(ex, "Lỗi hệ thống khi cập nhật thông tin OT.");
            }
        }
        ...
        int requestId, string reason, UserIdentityDto user, CancellationToken ct = default)
        {
            try
            {
                ...
                // Khác Leave (chỉ chặn khi IsFinalized): OT chặn hủy ngay khi CÓ BẤT KỲ cấp nào
                // đã quyết định (kể cả partial, chưa Finalized hẳn) — nghiệp vụ multi-level approval.
                var histories = await Uow.Repository<F03ApprovalHistory>().Query()
                    .Where(h => h.RequestId == requestId && h.RequestType == ModuleKind)
                    .ToListAsync(ct);

                bool anyDecided = histories.Any(h => h.Decision != DecisionType.Pending);
                if (anyDecided && !isAdmin)
                    return ServiceResult.Fail("Đơn đã có cấp duyệt xử lý, không thể tự hủy. Liên hệ Admin.");
        ...
        {
            base.ApplyCancel(entity, reason, user);   // set RequestStatus=Cancelled, IsActive=false
            entity.OTReasonSummary = $"[Cancelled by {user.EmployeeCode}] {reason}";
            entity.ModifiedBy = user.UserId;
            entity.ModifiedAt = DateTime.Now;
        }

        public async Task<ServiceResult> AddEmployeesAsync(
            int otRequestId, List<OTEmployeeDto> newEmployees, UserIdentityDto user, CancellationToken ct = default)
        {
            if (newEmployees == null || newEmployees.Count == 0)
                return ServiceResult.Fail("Không có nhân viên nào để thêm.");
            ...
            try
            {
                ...
                if (!isCreator && !IsAdmin(user))
                    return ServiceResult.Fail("Không có quyền thêm nhân viên vào đơn này.");
                ...
                entity.TotalOTHours = (await empRepo.Query()
                    .Where(x => x.OTRequestId == otRequestId && x.IsActive == true)
                    .SumAsync(x => (decimal?)x.OTHours, ct) ?? 0) + newEmployees.Sum(e => e.OTHours);
            ...
            {
                return InternalError(ex, "Lỗi hệ thống khi thêm nhân viên vào đơn.");
            }
        }
        // Không còn AttachFilesAsync ở đây — dùng bản chung từ BaseRequestCommandService
    }
}
