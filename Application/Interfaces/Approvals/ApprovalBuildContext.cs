...
public sealed class ApprovalBuildContext
{
    ...
    public int Year =>
        Extra.TryGetValue("Year", out var v) && v is int i ? i : DateTime.Today.Year;

    public string LeaveTypeCode =>
        Extra.TryGetValue("LeaveTypeCode", out var v) && v is string s ? s : "";
    ...
    public static ApprovalBuildContext ForTrip(
        string employeeCode, string deptCode, string positionCode) => new()
    {
        EmployeeCode = employeeCode,
        DeptCode = deptCode,
        PositionCode = positionCode
