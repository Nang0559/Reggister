...
/*
    Canonical policy seed.

    PositionCode values are HRM master keys. The INSERT is additionally
    restricted to positions that actually exist in F03Positions so deployment
    cannot fail with FK_F03ApprovalPolicies_F03Positions when an installation
    has a different HRM position master.
*/
...
FROM @Policies p
INNER JOIN dbo.F03Positions pos
    ON pos.PositionCode = p.PositionCode
WHERE NOT EXISTS
(
    SELECT 1
    FROM dbo.F03ApprovalPolicies x
