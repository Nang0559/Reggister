...
//        private bool _calendarNeedsInit = false;
...
//            {
//                _isLoading = false;
//                _calendarNeedsInit = true;
//            }
//        }

//        protected override async Task OnAfterRenderAsync(bool firstRender)
//        {
//            if (_calendarNeedsInit && !_calendarInitialized && _model != null)
//            {
//                _calendarNeedsInit = false; // ✅ reset ngay để tránh gọi lại nhiều lần
...
public partial class LeaveCreate : IAsyncDisposable
{
    ...
    private List<object> BuildCalendarEvents()
    {
        ...
        foreach (var d in _model?.LeaveDays ?? new())
        {
            ...
            events.Add(new
            {
                ...
                color,
                extendedProps = new { status = d.Status }
            });
        }
    ...
    public async Task OnEventClick(string eventId, string status)
    {
        ...
        var leaveItem = _model?.LeaveDays.FirstOrDefault(x => x.Id == eventId);
        if (leaveItem != null) await OpenDetailDialog(leaveItem);
    }
    ...
    private async Task OpenDetailDialog(HolidayViewModel item)
    {
        ...
        await DialogService.ShowAsync<LeaveDetailDialog>("Chi tiết đơn nghỉ",
            parameters,
