...
public sealed class EquipmentClientService : IEquipmentClientService
{
    ...
    public Task<ApiResponse<EquipmentRequestDto>> CreateRegistrationAsync(CreateEquipmentRegistrationDto request, CancellationToken ct = default) => Post("api/equipment/registrations", request, "create registration", ct);
    public Task<ApiResponse<EquipmentRequestDto>> SubmitRegistrationAsync(int id, CancellationToken ct = default) => Post($"api/equipment/registrations/{id}/submit", new { }, "submit registration", ct);
    ...
    public Task<ApiResponse<EquipmentRequestDto>> CreateRepairAsync(CreateEquipmentRepairDto request, CancellationToken ct = default) => Post("api/equipment/repairs", request, "create repair", ct);
    public Task<ApiResponse<EquipmentRequestDto>> SubmitRepairAsync(int id, CancellationToken ct = default) => Post($"api/equipment/repairs/{id}/submit", new { }, "submit repair", ct);
    ...
    { try { return await _http.GetAsync<T>(url, ct); } catch (OperationCanceledException) when (ct.IsCancellationRequested) { throw; } catch (Exception ex) { _logger.LogError(ex, "[EQUIPMENT_CLIENT] {Op}", op); return ApiResponse<T>.Fail("Không thể thực hiện thao tác thiết bị."); } }
    private async Task<ApiResponse<T>> Post<T>(string url, object body, string op, CancellationToken ct)
    { try { return await _http.PostAsync<T>(url, body, ct); } catch (OperationCanceledException) when (ct.IsCancellationRequested) { throw; } catch (Exception ex) { _logger.LogError(ex, "[EQUIPMENT_CLIENT] {Op}", op); return ApiResponse<T>.Fail("Không thể thực hiện thao tác thiết bị."); } }
}
