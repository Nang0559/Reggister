...
public sealed class OTClientService : IOTClientService
{
    ...
    public OTClientService(IHttpClientWithAuth http)
    {
        _http = http;
    }

    public Task<ApiResponse<object>> CreateOTRequestAsync(
        OTRequestUpsertDto request,
    ...
    private Task<ApiResponse<object>> SendApprovalAsync(
        string url,
        ApprovalActionDto request,
    ...
    public Task<ApiResponse<List<OTRequestDto>>> GetDeptOTByDateAsync(
        DateTime date,
        CancellationToken ct = default)
        => _http.GetAsync<List<OTRequestDto>>($"{Base}/dept-ot-by-date?date={date:yyyy-MM-dd}", ct);

    public Task<ApiResponse<List<OTEmployeeDto>>> GetDeptEmployeesAsync(
    ...
    public Task<ApiResponse<List<OTSummaryDto>>> GetMyHistoryAsync(
        int? year = null,
        CancellationToken ct = default)
    {
        var url = year.HasValue
            ? $"{Base}/history?year={year.Value}"
            : $"{Base}/history";
        return _http.GetAsync<List<OTSummaryDto>>(url, ct);
    }

    public Task<ApiResponse<OTBalanceDto>> GetEmployeeBalanceAsync(
        string employeeCode,
        int year,
        CancellationToken ct = default)
        => _http.GetAsync<OTBalanceDto>(
            $"{Base}/balance/{Uri.EscapeDataString(employeeCode)}/{year}/1", ct);

    public Task<ApiResponse<OTCombinedDataDto>> GetCombinedDataAsync(
        CancellationToken ct = default)
        => _http.GetAsync<OTCombinedDataDto>($"{Base}/combined-data", ct);

    public Task<ApiResponse<OTBalanceDto>> GetOTBalanceAsync(
        int year,
        CancellationToken ct = default)
        => _http.GetAsync<OTBalanceDto>($"{Base}/balance/{year}", ct);

    public Task<ApiResponse<List<OTSummaryDto>>> GetRecentOTRequestsAsync(
        int limit = 10,
        CancellationToken ct = default)
        => _http.GetAsync<List<OTSummaryDto>>($"{Base}/recent?limit={limit}", ct);

    public Task<ApiResponse<List<PendingApprovalItemDto>>> GetPendingApprovalsAsync(
        int level = 0,
        CancellationToken ct = default)
        => _http.GetAsync<List<PendingApprovalItemDto>>($"{Base}/pending?level={level}", ct);

    public Task<ApiResponse<PaginationResult<OTSummaryDto>>> GetPagedOTRequestsAsync(
        string? deptCode,
        string? status,
        DateTime? fromDate,
        DateTime? toDate,
    ...
    public Task<ApiResponse<object>> GetOTDashboardAsync(
        CancellationToken ct = default)
    ...
    public Task<ApiResponse<object>> UpdateEmployeeOTInfoAsync(
        int otRequestId,
        List<OTEmployeeDto> employees,
        CancellationToken ct = default)
        => _http.PutAsync<object>($"{Base}/{otRequestId}/employees/update", employees, ct);

    public Task<ApiResponse<OTRequestDto>> GetDetailAsync(
        int id,
        CancellationToken ct = default)
        => _http.GetAsync<OTRequestDto>($"{Base}/detail/{id}", ct);

    public Task<ApiResponse<object>> JoinOTRequestAsync(
        int id,
        CancellationToken ct = default)
        => _http.PostAsync<object>($"{Base}/join/{id}", new { }, ct);

    public Task<ApiResponse<object>> RemoveEmployeeAsync(
        int otRequestId,
        string employeeCode,
        CancellationToken ct = default)
        => _http.PostAsync<object>(
            $"{Base}/{otRequestId}/remove-employee/{Uri.EscapeDataString(employeeCode)}",
            new { }, ct);
}
